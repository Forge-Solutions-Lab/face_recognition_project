...
from members import load_members, find_member
...
def member_text(members, label, matched, field):
    m = find_member(members, label)
    if not matched:
        return "Unknown"
...
def predict_loop(cap, X, y, members):
    last = None
    while (frame := read_frame(cap)) is not None:
        ...
        draw_result(frame, box, member_text(members, label, matched, "name_en"), conf, dist, COLOR_OK if matched else COLOR_UNKNOWN)
        last = announce(member_text(members, label, matched, "name"), last)
...
def main():
    ...
    cap = open_camera()
    try:
        predict_loop(cap, X, y, load_members())
    finally:
        release_camera(cap)
